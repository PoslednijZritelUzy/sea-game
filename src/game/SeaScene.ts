...
export class SeaScene extends Phaser.Scene {
  ...
  private followSmoothRoute() {
    ...
    if (distance <= step) {
      ...
      this.shipPx += Math.cos(angle) * step;
      this.shipPy += Math.sin(angle) * step;
    }
  ...
  private buildCatmullRomRouteFromCurrentPosition(
    points: Point[]
  ): SmoothPoint[] {
